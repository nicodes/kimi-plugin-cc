...
function isProgressBlockTitle(line) {
  return ["Final output", "Assistant message", "Review output", "Kimi stderr (thinking/progress)"].includes(line);
}
...
function inferLegacyJobPhase(job, progressPreview = []) {
  ...
  for (let index = progressPreview.length - 1; index >= 0; index -= 1) {
    ...
    if (line.startsWith("starting kimi") || line.startsWith("resuming kimi session")) {
      return "starting";
    }
    if (line.startsWith("running tool:")) {
    ...
    if (line.startsWith("final answer captured")) {
      return "finalizing";
    }
    if (line.startsWith("kimi failed:")) {
      return "failed";
    }
  }
