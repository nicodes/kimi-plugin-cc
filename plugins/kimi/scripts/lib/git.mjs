...
// The review prompt travels to Kimi as a single argv element, so the inline diff
// budget must stay well under platform argv limits (Windows caps around 32 KB).
const DEFAULT_INLINE_DIFF_MAX_BYTES = 24 * 1024;
...
export function captureStatusSnapshot(cwd) {
  return gitChecked(cwd, ["status", "--short", "--untracked-files=all"]).stdout.trimEnd();
}
